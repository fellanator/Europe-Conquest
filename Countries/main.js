...
const countries =
{
       ...
       Austria:{
           ...
           Borders:[Countries.Italy, Countries.Germany, Countries.Slovenia, Countries.Hungary, Countries.Slovakia, Countries.Czechia,Countries.Switzerland],
           Coastal:false,
           Naval:1,
           Land:2.3,
           Air:1.8,
       ...
       Switzerland:{
           ...
           Borders:[Countries.France, Countries.Italy, Countries.Austria, Countries.Germany],
           Coastal:false,
           Naval:1,
           Land:1.5,
           Air:1.5,
...
const invaded = document.getElementById('invadedCountries');
...
/* takenCountries.forEach((country) =>{
    // these are for the option values
    const choice = document.createElement('option');
    choice.value = takenSelector;
    choice.textContent = country.Stats.Name;
    taken.appendChild(choice);
    takenSelector++;
    });

    invadedCountries.forEach((country) =>{
        // these are for the option values
        const choice = document.createElement('option');
        choice.value = invadeSelector;
        choice.textContent = country.Stats.Name;
        invaded.appendChild(choice);
        invadeSelector++;
        }); */
...
//variables for what country you selected, what countries you own, what countries you dont own, etc
...
if(imageSelector < 0)
{
    document.getElementById('countryImages').src = countryImagesList[46];
    imageSelector = 46;
...
next.addEventListener('click', function(){
    ...
    if(imageSelector > 46)
{
    document.getElementById('countryImages').src = countryImagesList[0];
    imageSelector = 0;
    imageStats.innerHTML = `Country Name: ${countriesList[imageSelector].Stats.Name} <br> Country Number: ${imageSelector + 1}`;
}else
{
...
/* function Determine(size, power, enemySize, enemyPower)
{
   const fullPop = size;
...
console.log(randomEnemyKills + " deaths");
   if(population <= 0 || enemyPopulation <= 0)
   {
    ...
    //fix random num the population from goin under zero
    //actually have the function return who wins
},1000)
}
